...
@gdclass
class ShowcaseSc(
    // These appear at the top of the inspector under "Base Stats".
    @export_group("Base Stats")
    @gdexport(ExportHint.range(1, 200, 1)) var damage: Int = 10,
    @gdexport(ExportHint.range(0.0, 1.0, 0.05)) var critChance: Double = 0.15
) extends Node2D:

    // Multiple annotations stack on the next var — Category then Group then export.
    // ── Appearance ────────────────────────────────────────────────────────────
    @export_category("Appearance")
    @export_group("Color & Style")
    @gdexport(ExportHint.colorNoAlpha) var tintColor: Color = Color(1f, 0.5f, 0.2f, 1f)
    @gdexport                          var rarity: Rarity         = Rarity.Common
    @export_group("Description")
    @gdexport(ExportHint.multiline) var notes: String = "Edit me in the inspector."
    ...
    @signal case class statsChanged(newDamage: Int, newCritChance: Double)
    ...
    override def _ready(): Unit =
        ...
        // Emit the typed signal (no-arg form — typed metadata already registered)
        emitSignal("stats_changed")
    ...
    @func def getOffset(): Vector2 = getPosition()

    /** Damage scaled by a factor — @func with Double param and return. */
    ...
    @func def applyTint(r: Double, g: Double, b: Double): Unit =
        tintColor = Color(r.toFloat, g.toFloat, b.toFloat, 1f)
