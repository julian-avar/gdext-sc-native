...
object Ast:
    ...
    case class KindEnumValue(index: Int, name: String, descriptionOption: Option[String])
    case class KindStructMember(name: VarName, description: TypeDescription)

    enum Kind(val name: String) extends Ast:
        case Enum(values: Vector[KindEnumValue], isBitfield: Boolean) extends Kind("Enum")
        case Handle(isConst: Boolean, isUninitialized: Boolean, parent: Option[TypeName])
            extends Kind("Handle")
        case Alias(`type`: TypeName)                   extends Kind("Alias")
        case Struct(members: Vector[KindStructMember]) extends Kind("Struct")
