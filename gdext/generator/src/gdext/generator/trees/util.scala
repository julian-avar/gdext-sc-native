...
private def packArg(arg: Ast.GodotArg, i: Int, vb: Set[String] = Set.empty): (List[Stat], Term) =
    ...
    arg.typeName match
        ...
        case "StringName" =>
            ...
            val cached = Term.Apply(
              Term.Select(Term.Name("StringNames"), Term.Name("cached")),
              List(param)
            )
            (Nil, cached)
...
private def retSetup(
    ...
    case t if vb.contains(t) =>
        ...
        // malloc(T.byteSize).asInstanceOf[T] — byteSize comes from the companion
...
def newEngineAnonymous(typeName: String, argss: List[List[Term]]): Term =
    ...
    val argReads: List[Stat] = m.args.zipWithIndex.map { (a, i) =>
        ...
        val read: Term = a.typeName match
            ...
            case t if valueBuiltins.contains(t) =>
                asInstanceOfTerm(argI, Type.Name(t))
    ...
    val retLines: List[Stat] = m.returnTypeName match
        ...
        case t
            if t == "String" || t == "StringName" || t == "PackedStringArray" || t == "Array" ||
                ...
                valueBuiltins.contains(t) => List(callExpr)
        case _ => List(
              simpleValDef("_r", callExpr),
              derefAssign(
...
private def buildTypedCall(
    ...
    val bindExpr = Term.Select(
      Term.Select(Term.Name(cls.name), Term.Name("Binds")),
      Term.Name(toCamel(m.name))
    )
...
private def methodBody(
    ...
    buildTypedCall(cls, m, isStatic).getOrElse {
        ...
        val body = setupArgStats(m.args, valueBuiltins) ++ rAlloc.toList ++ List(callStat) ++
            rRead.toList
        val needsZone = m.args.filterNot(_.hasDefault).exists(_.typeName == "String")
