...
object TreesGenerator:
    ...
    private def derefAssign(lhs: Term, rhs: Term): Stat =
        Term.Assign(Term.Apply(lhs, List(Lit.Long(0L))), rhs)
        ...
        def typesSource(kindName: String, types: Vector[Ast.Type]): String =
            ...
            val stats: List[Stat] = types.flatMap { tp =>
                ...
                tp.kind match
                    ...
                    case Ast.Kind.Function(args, ret) => List(withComment(
                          comment,
                          functionDefStat(tp.name, Ast.Kind.Function(args, ret))
                        ))
                end match
            }.toList
    ...
    object InterfaceGen:
        def interfaceSource(interfaces: Vector[Ast.Interface]): String =
            ...
            val typeDefs: List[Stat] = interfaces.map { iface =>
                ...
                if comment.nonEmpty then s"$comment\n${typeStat.syntax}".parse[Stat].get
                else typeStat
            }.toList
            ...
            val resultParam = Term
                .Param(Nil, Term.Name("result"), Some(Type.Name("Interface")), None)
        ...
        def builtinsSource(
            ...
            def buildTagGiven(name: String, members: Vector[Ast.BuiltinMember]): Defn.GivenAlias =
                ...
                val materialize = Term.ApplyType(
                  Term.Select(
                    Term.Name("Tag"),
                    Term.Name(s"materializeCStruct${members.length}Tag")
                  ),
            ...
            def buildFieldExtMethods(
                name: String,
                members: Vector[Ast.BuiltinMember]
            ): List[Defn.Def] = members.zipWithIndex.flatMap { (m, i) =>
                val idx = i + 1
                val fn  = m.name
                ...
                def mapBody: Term =
                    val sets = fields.map { f =>
                        Term.Assign(
                          ...
                          Term.Apply(
                            Term.Name("f"),
                            List(Term.Select(Term.Name("v"), Term.Name(f)))
                          )
                        )
                    }
                    Term.Block(
            ...
            val valueStats: List[Stat] = valueTypes.flatMap { b =>
                ...
                val objectStats: List[Stat] = List(buildTagGiven(b.name, b.members)) ++
                    buildApplyCtor(b.name, b.members).toList ++ List(buildExtensionGroup(
                      extParam,
                      Nil,
                      buildFieldExtMethods(b.name, b.members) ++
        ...
        def wrapperSource(
                ...
                cls.methods.find { m =>
                    ...
                    val setterDef = p.setter.flatMap { sName =>
                        cls.methods.find { m =>
                            ...
                            val spt = paramGodotType(
                              sm.args.head.typeName,
                              sm.args.head.meta,
                              valueBuiltins
                            )
                            Defn.Def(
            ...
            val singletonStats: List[Stat] =
                if !cls.isSingleton then Nil
                    ...
                    singletonVal ::
                        instanceMethods.map(buildForwardingMethod(cls, _, valueBuiltins)).toList
            ...
            val parentInits =
                List(Init(Type.Name(parentName), Name.Anonymous(), parentArgs))
            ...
            val nodeExtStat: Option[Defn.ExtensionGroup] = Option.when(cls.name == "Node") {
                val scParam = Term
                    .Param(Nil, Term.Name("sc"), Some(Type.Name("StringContext")), None)
            ...
            val allImports =
                if cls.name == "Node" then
                    baseImports :+ "import scala.scalanative.libc.string.memset"
                else baseImports
            "// Generated by gdext generator — do not edit.\n" +
    ...
    object UtilityGen:
        def utilitySource(
            ...
            val methodDefs: List[Defn.Def] = utilities.map { fn =>
                ...
                val reqArgs =
                    if fn.isVararg then Vector.empty else fn.arguments.filterNot(_.hasDefault)
                val params: List[Term.Param] =
                    if fn.isVararg then
                        List(Term.Param(Nil, Term.Name("args"), Some(ptrPtrByte), None))
                    else reqArgs.map(godotParam(_, valueBuiltins)).toList
                val (rAlloc, rRead) =
                    retSetup(fn.returnTypeName, None, valueBuiltins, refcountedTypes)
                val retPtr: Term =
                ...
                val body = setupArgStats(fn.arguments, valueBuiltins, fn.isVararg) ++
                    rAlloc.toList ++ List(callStat) ++ rRead.toList
    ...
    object GlobalScopeGen:
        private def enumObject(e: Parser.GlobalEnum): Defn.Object =
        ...
        def globalScopeSource(
            ...
            val methodDefs: List[Defn.Def] = varargVoidFns.map { fn =>
                ...
                Defn.Def(Nil, Term.Name(name), Nil, List(List(param)), Some(Type.Name("Unit")), zoneWrap(body))
            }.toList
