...
object GodotClass:
    /** Derive a `GodotClass[T]` for a user-defined extension class at compile time.
      ...
      * `isRefCounted = false` (correct for Node subclasses), and wraps pointers by creating a
      * fresh instance and setting its `ptr` field.
      *
    ...
    private def derivedImpl[T <: GodotObject: Type](using Quotes): Expr[GodotClass[T]] =
        ...
        if requiredParams.nonEmpty then
            report.errorAndAbort(
              s"GodotClass.derived[${sym.name}]: primary constructor must have all-default params. " +
