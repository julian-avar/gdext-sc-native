...
object Tres:
    ...
    extension [T <: GodotObject](t: Tres[T])
        def gd: Gd[T] = t
...
/** A typed scene reference for `@gdexport` fields.
  *
  * Shows a scene picker in the inspector. `T` is the expected root node type of the scene.
  * Use `Tscn[Node]` for any scene; use a concrete type like `Tscn[Player]` to signal the
  * intended root type (Godot does not enforce this at edit time).
  *
  * {{{
...
object Tscn:
    ...
    extension [T <: GodotObject](t: Tscn[T])
        def gd: Gd[T] = t
