...
/** Type metadata needed to export a Scala field to the Godot inspector.
  ...
  *
  * Givens for Godot builtin math types (Vector2, Color, etc.) live in
  * `gdext.generated.ExportTypes` and are imported with `gdext.generated.*`.
  */
trait ExportType[A]:
    def variantType: Int
...
object ExportType:
    /** Export a direct engine class reference (e.g. `@gdexport var target: Node2D`).
      *
      * Shown in the Godot inspector as a NodeType property slot filtered to `className`. The
      * `wrap` call on read creates a fresh Scala wrapper around the engine object — state lives in
      * Godot so this is safe for engine classes. User-defined class exports work but return a
      * fresh (stateless) wrapper until identity preservation lands in a later phase.
      */
        ...
        new ExportType[T]:
            ...
            def write(dest: Ptr[Byte], value: T): Unit =
                ...
                !(dest + 8).asInstanceOf[Ptr[Ptr[Byte]]] =
                    if value == null then null else value.ptr
