...
/** Factory methods for creating Godot Callables from Scala lambdas.
  *
  * Use `Callable.lambda` to wrap a Scala closure for engine APIs that take a `Callable` (e.g.
  * `Tween.tween_callback`, `SceneTree.create_timer`, custom signals).
  *
  * {{{
  * // Tween callback:
  ...
  * }}}
  *
  * **Lifetime:** `Callable.lambda` allocates a heap Callable. Godot holds a reference while it has
  * the callable (e.g. inside a Tween). For fire-once callables this is effectively self-managing.
  * For long-lived connections prefer `GodotObject.connect` which provides a `ConnectionToken` for
  * explicit cleanup.
  */
object CallableLambda:
    ...
    def apply(f: () => Unit): Callable = make(CallbackRegistry.register(new Callback0(f)))

    def apply[A: FromVariant](f: A => Unit): Callable =
        make(CallbackRegistry.register(new Callback1[A](f, summon[FromVariant[A]])))

    def apply[A: FromVariant, B: FromVariant](f: (A, B) => Unit): Callable = make(
      CallbackRegistry
          .register(new Callback2[A, B](f, summon[FromVariant[A]], summon[FromVariant[B]]))
    )

    def apply[A: FromVariant, B: FromVariant, C: FromVariant](f: (A, B, C) => Unit): Callable =
        make(CallbackRegistry.register(new Callback3[A, B, C](
          f,
