...
abstract class GodotObject:
    ...
    /** Disconnect a signal connection. Frees the trampoline allocation and removes the callback
      * from [[CallbackRegistry]]. Always call this in `_exitTree` for cross-node connections.
      */
    def disconnect(token: ConnectionToken): Unit =
        CallbackRegistry.remove(!token.udBuf.asInstanceOf[Ptr[Int]])
