...
object Generator:
    ...
    def types(types: Vector[Ast.Type]): Vector[ScalaFile] = types.groupBy(_.kind.name)
        .map { (name, ts) =>
            ScalaFile(
              content = TreesGenerator.typesSource(name, ts),
    ...
    def interfaces(interfaces: Vector[Ast.Interface]): Vector[ScalaFile] = Vector(ScalaFile(
      content = TreesGenerator.interfaceSource(interfaces),
    ...
    def generateBuiltins(builtins: Vector[Ast.BuiltinClass]): Vector[ScalaFile] =
        ...
        Vector(ScalaFile(
          content = TreesGenerator.builtinsSource(valueTypes, heapTypes),
          path = "gdext/generated",
          name = "GodotBuiltins"
        ))
    ...
    def classVirtuals(
        ...
        classes.flatMap { cls =>
            ...
            if virtuals.isEmpty then None
                ...
                val entries = TreesGenerator.virtualEntryTerms(virtuals, valueBuiltins)
                if entries.isEmpty then None
                else
                    Some(ScalaFile(
                      content = TreesGenerator.virtualsSource(cls, entries),
    ...
    def generateWrappers(
        ...
        ScalaFile(
          content = TreesGenerator.wrapperSource(cls, valueBuiltins, refcountedTypes),
    ...
    def generateUtilityFunctions(
      ...
      content = TreesGenerator.utilitySource(utilities, valueBuiltins, refcountedTypes),
      path = "gdext/generated",
