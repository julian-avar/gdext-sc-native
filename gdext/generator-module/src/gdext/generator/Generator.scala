...
object Generator:
    ...
    def generateBuiltins(builtins: Vector[Ast.BuiltinClass]): Vector[ScalaFile] =
        ...
        def fieldExtensionMethods(members: Vector[Ast.BuiltinMember]): Vector[String] = members
            .zipWithIndex.flatMap { (m, i) =>
                val idx = i + 1
        ...
        val valueDefs = valueTypes.map { b =>
            ...
            val methods  = fieldExtensionMethods(b.members) ++
                mathExtensionMethods(b.name, b.members)
    ...
    private def packArg(
        ...
        arg.typeName match
            ...
            // Object args: ptrcall reads *args[i] as a GDObjectPtr, so store .ptr in a slot.
            case _ => (
                  s"val $slot = stackalloc[Ptr[Byte]](); !$slot = $param.ptr",
                  s"$slot.asInstanceOf[Ptr[Byte]]"
                )
        end match
    end packArg
    ...
    def generateWrappers(
        ...
        val selfNodeGiven =
            if cls.name == "Node" then Seq("given selfNode: Node = this") else Seq.empty
        val classBody =
            if bodyParts.isEmpty && ptrInit.isEmpty && selfNodeGiven.isEmpty then Seq.empty
            else
                val parts = selfNodeGiven ++ bodyParts
                if ptrInit.nonEmpty then ptrInit +: parts else parts

        val nodeMemsetImport =
            if cls.name == "Node" then "\nimport scala.scalanative.libc.string.memset" else ""

        val nodeExtension =
            if cls.name == "Node" then """
                |extension (sc: StringContext)(using node: Node)
                |  def $(args: Any*): Node = Zone {
        ...
        val content =
            s"""|// Generated by gdext generator — do not edit.
                ...
                |import gdext.core.{GdxApi, GodotObject}$nodeMemsetImport
                |
                |$classDef ${
                ...
                |""".stripMargin + (if companionBody.nonEmpty then s"""|
                    ...
                    |}
                    |""".stripMargin else "") + nodeExtension
